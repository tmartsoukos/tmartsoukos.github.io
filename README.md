...
Μία κοινή ομάδα: **όποιος συνδεθεί βλέπει και αλλάζει τα ίδια δεδομένα**. Δεν υπάρχουν
κωδικοί πρόσκλησης ούτε ρόλοι — ο βοηθός προπονητή φτιάχνει λογαριασμό, συνδέεται και
δουλεύει στα ίδια δεδομένα σε πραγματικό χρόνο. Η ομάδα δημιουργείται μόνη της στην πρώτη
σύνδεση, μέσω του RPC `default_team()`.

Το publishable/anon key ενσωματώνεται στο bundle του browser — αυτό είναι αναμενόμενο. Η
προστασία στηρίζεται στα RLS policies: όλοι οι πίνακες έχουν RLS και **όλα τα policies
αφορούν αποκλειστικά τον ρόλο `authenticated`**. Ο ρόλος `anon` (μη συνδεδεμένος
επισκέπτης) δεν έχει κανένα policy, άρα δεν διαβάζει και δεν γράφει τίποτα. Οι έτοιμες
ασκήσεις (`is_preset`) είναι μόνο για ανάγνωση.
